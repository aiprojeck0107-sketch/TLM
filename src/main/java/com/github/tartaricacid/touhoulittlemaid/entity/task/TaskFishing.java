...
public class TaskFishing implements IMaidTask {
    ...
    public FunctionCallSwitchResult onFunctionCallSwitch(EntityMaid maid) {
        if (maid.getMainHandItem()/*.canPerformAction(ToolActions.FISHING_ROD_CAST)*/.getItem() instanceof FishingRodItem) {
            return FunctionCallSwitchResult.NO_CHANGE;
        }
        if (TaskEquipUtil.tryEquipFromBackpack(maid, item -> item.getItem() instanceof FishingRodItem /*item.canPerformAction(ToolActions.FISHING_ROD_CAST)*/)) {
