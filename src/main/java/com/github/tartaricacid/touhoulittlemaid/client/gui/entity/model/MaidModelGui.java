...
public class MaidModelGui extends AbstractModelGui<EntityMaid, MaidModelInfo> {
    ...
    protected void notifyModelChange(EntityMaid maid, MaidModelInfo info) {
        if (info.getEasterEgg() == null) {
            FriendlyByteBuf buf = MaidModelMessage.encode(maid.getId(), info.getModelId());
            ClientPlayNetworking.send(MaidModelMessage.ID, buf);
            String useSoundPackId = info.getUseSoundPackId();
            if (StringUtils.isNotBlank(useSoundPackId)) {
                ClientPlayNetworking.send(SetMaidSoundIdMessage.ID, SetMaidSoundIdMessage.encode(maid.getId(), useSoundPackId));
            }
            // 切换模型时，重置手部动作
    ...
    protected void onClickCloseButton() {
        if (this.entity != null) {
            ClientPlayNetworking.send(OpenMaidGuiMessage.ID, OpenMaidGuiMessage.encode(this.entity.getId()));
        }
    }
}
