...
public class CChessToClientMessage {
    public static final ResourceLocation ID = getResourceLocation("cchess_to_client");

    public static FriendlyByteBuf encode(BlockPos centerPos, String fenData) {
    ...
    private static void onHandle(BlockPos pos, String fenData) {
        ...
        FriendlyByteBuf buf = CChessToServerMessage.encode(pos, moveFinal, maidLost, playerLostFinal);
        Minecraft.getInstance().submitAsync(() -> ClientPlayNetworking.send(CChessToServerMessage.ID, buf));
    }
}
